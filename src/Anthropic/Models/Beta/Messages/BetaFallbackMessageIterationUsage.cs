...
///
/// <para>The terminal entry of a fallback-served turn: when a fallback hop's output
/// is the returned message, the entry for the iteration that completed it carries
/// this type in place of `message`. A declined hop and the serving hop's earlier
/// tool-loop iterations produce `message` entries. Whether a fallback model served
/// the response is signalled by the presence of this entry in `usage.iterations`.</para>
/// </summary>
[JsonConverter(
