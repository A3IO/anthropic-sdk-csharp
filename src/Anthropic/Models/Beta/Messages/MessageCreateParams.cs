...
///
/// <para>Learn more about the Messages API in our [user guide](https://platform.claude.com/docs/en/get-started)</para>
///
...
public record class MessageCreateParams : ParamsBase
{
    ...
    /// </summary>
    public FallbackCreditToken? FallbackCreditToken
    {
        get
        {
            this._rawBodyData.Freeze();
            return this._rawBodyData.GetNullableClass<FallbackCreditToken>("fallback_credit_token");
        }
    ...
    /// </summary>
    public BetaFallbacksParam? Fallbacks
    {
        get
        {
            this._rawBodyData.Freeze();
            return this._rawBodyData.GetNullableClass<BetaFallbacksParam>("fallbacks");
        }
        init { this._rawBodyData.Set("fallbacks", value); }
    }
...
/// the partial text — with any trailing whitespace stripped from the final text block
/// — and paired server-tool blocks streamed before the refusal; the appended-assistant
/// form is not available for requests with `output_format` set or forced `tool_choice`),
/// on an eligible fallback model, on the same platform, and within 5 minutes of the
/// refusal; a mismatch is a 400. A token minted mid-server-tool-loop whose partial
/// content was continuable may only be redeemed with the appended-assistant form
/// — if an exact-body retry is rejected with a 400 saying the token must be redeemed
/// by continuing the partial response, retry with the appended-assistant form instead.</para>
///
/// <para>When the appended-assistant form is used on a model that otherwise disallows
/// assistant-turn prefill, this token also authorizes that one prefill.</para>
/// </summary>
[JsonConverter(typeof(FallbackCreditTokenConverter))]
public record class FallbackCreditToken : ModelBase
{
    public object? Value { get; } = null;
    ...
    public FallbackCreditToken(string value, JsonElement? element = null)
    {
        this.Value = value;
        this._element = element;
    }

    public FallbackCreditToken(BetaFallbackCreditTokenParam value, JsonElement? element = null)
    {
        this.Value = value;
        this._element = element;
    }

    public FallbackCreditToken(JsonElement element)
    {
        this._element = element;
    }
    ...
    /// <summary>
    /// Returns true and sets the <c>out</c> parameter if the instance was constructed with a variant of
    /// type <see cref="BetaFallbackCreditTokenParam"/>.
    ///
    /// <para>Consider using <see cref="Switch"/> or <see cref="Match"/> if you need to handle every variant.</para>
    ///
    /// <example>
    /// <code>
    /// if (instance.TryPickBetaFallbackCreditTokenParam(out var value)) {
    ///     // `value` is of type `BetaFallbackCreditTokenParam`
    ///     Console.WriteLine(value);
    /// }
    /// </code>
    /// </example>
    /// </summary>
    public bool TryPickBetaFallbackCreditTokenParam(
        [NotNullWhen(true)] out BetaFallbackCreditTokenParam? value
    )
    {
        value = this.Value as BetaFallbackCreditTokenParam;
        return value != null;
    }
    ...
    /// instance.Switch(
    ///     (string value) =&gt; {...},
    ///     (BetaFallbackCreditTokenParam value) =&gt; {...}
    /// );
    /// </code>
    /// </example>
    /// </summary>
    public void Switch(
        System::Action<string> @string,
        System::Action<BetaFallbackCreditTokenParam> betaFallbackCreditTokenParam
    )
    {
        switch (this.Value)
        {
            case string value:
                @string(value);
                break;
            case BetaFallbackCreditTokenParam value:
                betaFallbackCreditTokenParam(value);
                break;
            default:
                throw new AnthropicInvalidDataException(
                    "Data did not match any variant of FallbackCreditToken"
                );
        }
    }
    ...
    public T Match<T>(
        System::Func<string, T> @string,
        System::Func<BetaFallbackCreditTokenParam, T> betaFallbackCreditTokenParam
    )
    {
        return this.Value switch
        {
            string value => @string(value),
            BetaFallbackCreditTokenParam value => betaFallbackCreditTokenParam(value),
            _ => throw new AnthropicInvalidDataException(
                "Data did not match any variant of FallbackCreditToken"
            ),
        };
    ...
    public static implicit operator FallbackCreditToken(string value) => new(value);

    public static implicit operator FallbackCreditToken(BetaFallbackCreditTokenParam value) =>
        new(value);
    ...
    public override void Validate()
    {
        ...
        this.Switch(
            (_) => { },
            (betaFallbackCreditTokenParam) => betaFallbackCreditTokenParam.Validate()
        );
    }

    public virtual bool Equals(FallbackCreditToken? other) =>
        other != null
        && this.VariantIndex() == other.VariantIndex()
        && JsonElement.DeepEquals(this.Json, other.Json);
    ...
    int VariantIndex()
    {
        ...
        {
            string _ => 0,
            BetaFallbackCreditTokenParam _ => 1,
            _ => -1,
        };
    }
}

sealed class FallbackCreditTokenConverter : JsonConverter<FallbackCreditToken?>
{
    public override FallbackCreditToken? Read(
        ref Utf8JsonReader reader,
        System::Type typeToConvert,
        JsonSerializerOptions options
    )
