...
/// </summary>
public record class DreamCreateParams : ParamsBase
{
    ...
    public required IReadOnlyList<BetaDreamInput> Inputs
    {
        get
        {
            ...
            return this._rawBodyData.GetNotNullStruct<ImmutableArray<BetaDreamInput>>("inputs");
        }
        init
        {
            this._rawBodyData.Set<ImmutableArray<BetaDreamInput>>(
                "inputs",
    ...
    public DreamCreateParams() { }

#pragma warning disable CS8618
    [SetsRequiredMembers]
    public DreamCreateParams(DreamCreateParams dreamCreateParams)
        : base(dreamCreateParams)
    {
        this._rawBodyData = new(dreamCreateParams._rawBodyData);
    }
...
#pragma warning restore CS8618
    ...
    public static DreamCreateParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawHeaderData,
        IReadOnlyDictionary<string, JsonElement> rawQueryData,
        IReadOnlyDictionary<string, JsonElement> rawBodyData
    )
    ...
    public virtual bool Equals(DreamCreateParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override System::Uri Url(ClientOptions options)
    {
        ...
        return new System::UriBuilder(options.BaseUrl.ToString().TrimEnd('/') + "/v1/dreams")
        {
...
public record class Model : ModelBase
{
    ...
    public Model(BetaDreamModelConfigParam value, JsonElement? element = null)
    {
        this.Value = value;
        this._element = element;
    }
    ...
    /// <summary>
    /// Returns true and sets the <c>out</c> parameter if the instance was constructed with a variant of
    /// type <see cref="BetaDreamModelConfigParam"/>.
    ///
    /// <para>Consider using <see cref="Switch"/> or <see cref="Match"/> if you need to handle every variant.</para>
    ///
    /// <example>
    /// <code>
    /// if (instance.TryPickBetaDreamModelConfigParam(out var value)) {
    ///     // `value` is of type `BetaDreamModelConfigParam`
    ///     Console.WriteLine(value);
    /// }
    /// </code>
    /// </example>
    /// </summary>
    public bool TryPickBetaDreamModelConfigParam(
        [NotNullWhen(true)] out BetaDreamModelConfigParam? value
    )
    {
        value = this.Value as BetaDreamModelConfigParam;
        return value != null;
    }
    ...
    /// instance.Switch(
    ///     (string value) =&gt; {...},
    ///     (BetaDreamModelConfigParam value) =&gt; {...}
    /// );
    /// </code>
    /// </example>
    /// </summary>
    public void Switch(
        ...
        System::Action<BetaDreamModelConfigParam> betaDreamModelConfigParam
    )
    {
        switch (this.Value)
        {
            case string value:
                @string(value);
                break;
            case BetaDreamModelConfigParam value:
                betaDreamModelConfigParam(value);
                break;
            default:
    ...
    public T Match<T>(
        ...
        System::Func<BetaDreamModelConfigParam, T> betaDreamModelConfigParam
    )
    {
        return this.Value switch
        {
            string value => @string(value),
            BetaDreamModelConfigParam value => betaDreamModelConfigParam(value),
    ...
    public static implicit operator Model(BetaDreamModelConfigParam value) => new(value);
    ...
    public override void Validate()
    {
        ...
        this.Switch(
            (_) => { },
            (betaDreamModelConfigParam) => betaDreamModelConfigParam.Validate()
        );
    }
